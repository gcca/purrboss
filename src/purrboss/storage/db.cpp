...
class Connection {
public:
  Connection(const std::string &path, int flags, const char *setup_pragmas) {
...
Database Database::ReadOnly(std::string path) {
  return Database{std::move(path), SQLITE_OPEN_READONLY | SQLITE_OPEN_FULLMUTEX,
                  "PRAGMA foreign_keys = ON;"};
}

Database Database::ReadWrite(std::string path) {
  return Database{std::move(path),
...
Database::Database(std::string path, int open_flags, const char *setup_pragmas)
    : path_{std::move(path)}, open_flags_{open_flags},
      setup_pragmas_{setup_pragmas} {}

void Database::Initialize() const {
  Connection connection{path_, open_flags_, setup_pragmas_};
...
Database::FindAuthUser(std::string_view username) const {
  Connection connection{path_, open_flags_, setup_pragmas_};
  Statement statement{connection.get(),
  ...
  return AuthUser{
      .username = statement.Text(0),
      .password = statement.Text(1),
      .email = statement.Text(2),
      .is_active = statement.Integer(3) != 0,
      .created_at = statement.Integer(4),
      .last_logged_at = statement.Integer(5),
  };
}

void Database::UpdateLastLogin(std::string_view username,
                               std::int64_t when) const {
  Connection connection{path_, open_flags_, setup_pragmas_};
  Statement statement{
      connection.get(),
      "UPDATE auth_user SET last_logged_at = ? WHERE username = ?"};
  statement.BindInteger(1, when);
